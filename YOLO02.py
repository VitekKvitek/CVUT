...
def main(params, yolo_net):
    ...
    for i in range(0, len(dataloader)):
        ...
        #results is 3 dimensional
        #1st dimension is frame of video ?? mby

        #print (" ")
        #print(f"Yolo v5 results are: {results.xyxy}")
        #print (" ")
        ...
        #draw_bbox(image_path,out_dir,x1,y1,x2,y2)
        break
...
def write_image_info_to_csv(x1,y1,x2,y2,csv_file_name,image_path):
    ...
    #print(f"I am writing this to csv sheat :{data}")
...
def get_bbox_coords(results):
        ...
        if not current_biggest_bbox_area == 0:
            ...
            #print (" ")
            #print (f"bbox cors: {x1}  {y1} {x2} {y2}")
            #print (" ")

            return x1,y1,x2,y2
        else:
            #print("Did not go threw selection")
...
if __name__ == "__main__":
    ...
    source_folder = r"D:\vita\car_data_set\resource\TCCPD"
    image_list = os.listdir(source_folder)
    

    for data_source in tqdm(image_list):
        params = {
            "out_dir": r"D:\vita\car_data_set\resource\TCCPD_filtered",
            "data_source": data_source,
            "image_path": os.path.join(source_folder, data_source),
            "size": (480, 640),
            "raft_iter": 6,
            "csv_file_name": "TCCPD.csv",
            "trash_folder": r"D:\vita\car_data_set\resource\TCCPD_trash"
        }
        #calib(params)
